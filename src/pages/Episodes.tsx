...
import { Episodes as FetchEpisodes } from '../service/axios'
...
const Episodes = () => {
    ...
    const fetchEpisodes = async () => {
        if (page !== null) {
            const data = await FetchEpisodes.GET(page)
        ...
        <Center as='main' justifyContent='flex-start' width='70%'>
            {episodes?.map((episode, index) => {
                if (index === episodes.length - 1)
                    return <EpisodeBox viewRef={ref} key={episode.id} episode={episode} />
                else return <EpisodeBox key={episode.id} episode={episode} />
            })}
        </Center>
    )
}
